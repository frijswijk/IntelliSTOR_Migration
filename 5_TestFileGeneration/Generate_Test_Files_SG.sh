#!/bin/bash
# Generate Test Files - Singapore
# macOS equivalent of Generate_Test_Files_SG.bat
...
LOG_FILE="Generate_Test_Files_SG_LOG.txt"

echo "Generate_Test_Files_SG Script started at: ${START_TIME}"
echo "-----------------------------------------------------------------------"
...
mkdir -p "${TestGen_TargetFolder_SG}"

# Run the Generate_Test_Files script
python3 Generate_Test_Files.py --ReportSpecies "${TestGen_ReportSpecies_SG}" --FolderExtract "${TestGen_FolderExtract_SG}" --TargetFolder "${TestGen_TargetFolder_SG}" --Number "${TestGen_MaxSpecies}" --quiet
...
# --- 3. Logging Section ---
echo "[${START_TIME}] Country: SG | Max Species: ${TestGen_MaxSpecies} | Target: ${TestGen_TargetFolder_SG} | Duration: ${DURATION}" >> "${LOG_FILE}"
