...
REM Usage: apply_watermark_wrapper.bat <input.pdf> <output.pdf> <watermark.png> <rotation> <opacity>
...
"%MAGICK%" "%WATERMARK_IMG%" -resize 300 -rotate %ROTATION% "%PROCESSED_WM%"
if errorlevel 1 (
...
"%MAGICK%" -size 612x792 xc:none "%PROCESSED_WM%" -gravity center -compose dissolve -define compose:args=%OPACITY% -composite "%WATERMARK_PDF%"
if errorlevel 1 (
