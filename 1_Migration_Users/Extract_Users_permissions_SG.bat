...
set "START_TIME=%TIME%"
set "LOG_FILE=Extract_Users_Permissions_SG_LOG.txt"
set FLAG=
rem set FLAG=--TESTDATA

echo Extract_users_Permissions_SG Script started at: %DATE% %START_TIME%
echo -----------------------------------------------------------------------

rem python extract_users_permissions.py --server localhost --database iSTSGUAT --windows-auth --output Users_SG
python Extract_Users_Permissions.py --server %SQLServer% --database %SQL-SG-Database% --output %Users-SG% --windows-auth --quiet %FLAG%
