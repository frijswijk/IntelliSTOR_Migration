cls
@Echo off
rem python Extract_Users_Permissions.py --server localhost --database iSTMYUAT --windows-auth --quiet --output Users_MY  --TESTDATA

call ..\Migration_Environment.bat
:: --- 1. Capture Start Time ---
set "START_TIME=%TIME%"
set "LOG_FILE=Extract_Users_Permissions_MY_LOG.txt"
set FLAG=
rem set FLAG=--TESTDATA

echo Extract_users_Permissions_MY Script started at: %DATE% %START_TIME%
echo -----------------------------------------------------------------------

rem python extract_users_permissions.py --server localhost --database iSTSGUAT --windows-auth --output Users_SG
python Extract_Users_Permissions.py --server %SQLServer% --database %SQL-MY-Database% --output %Users-MY% --windows-auth --quiet %FLAG%
...
echo [%DATE% %START_TIME%] Country: MY ^| DB: %SQL-MY-Database% ^| Flag: %FLAG% ^| Duration: %DURATION% >> "%LOG_FILE%"
