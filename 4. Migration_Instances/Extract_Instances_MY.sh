...
python3 Extract_Instances.py --server "${SQLServer}" --database "${SQL_MY_Database}" --user "${SQLUser}" --password "${SQLPassword}" --input "${Instances_Input_MY}" --output "${Instances_Output_MY}" --start-year "${Instances_StartYear_MY}" --quiet
